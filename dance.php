...
<body>
    ...
    <div class="container">
        ...
        if ($_SERVER["REQUEST_METHOD"] != "POST" || isset($_POST['Username']) && isset($_POST['Password2'])) {
            ...
            ?>
            <form action="dance.php" class="form1" method="post" id="form1">
                <h1>Join as</h1>
        ...
        <div id="message">
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                ...
                if ($result->num_rows == 1) {
                    ...
                    $checkStmt = $conn->prepare("SELECT * FROM Dance WHERE Username = ?");
                    $checkStmt->bind_param("s", $userData['Username']);
                    $checkStmt->execute();
                    $checkResult = $checkStmt->get_result();
                
                    if ($checkResult->num_rows > 0) {
                        echo '<div class="error">You have already joined the Dance events.</div>';
                    } else {
                        // Prepare the insert statement for the Cricket table
                        $insertStmt = $conn->prepare("INSERT INTO `pbl`.`Dance` (`Username`, `FirstName`, `LastName`, `Branch`, `Year`, `Password2`, `Email`, `ContactNo`, `DateTime`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())");
                        ...
                        echo '<div class="success">You have joined the Dance events successfully.</div>';
                    }
                } else {
